...
type Handlers struct {
	...
	SectTech          *handler.SectTechHandler
	SectWarehouse     *handler.SectWarehouseHandler
	SectTechnique     *handler.SectTechniqueHandler
...
func Bootstrap(mongoDB *mongo.Database, rdb *redis.Client, playerAddr string) *Handlers {
	...
	return &Handlers{
		...
		SectTech:      handler.NewSectTechHandler(sectTechSvc),
		SectWarehouse: handler.NewSectWarehouseHandler(sectWarehouseSvc),
		SectTechnique: handler.NewSectTechniqueHandler(sectTechniqueSvc),
