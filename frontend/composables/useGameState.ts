...
export function useGameState() {
  ...
  function openMenu(m: any) { activeMenu.value = m; activeSub.value = m.children?.[0]?.key || m.key; modalDesc.value = descs[activeSub.value] || ''; if (!['map', 'profession', 'pigeon', 'backpack'].includes(m.key)) modalVisible.value = true }
