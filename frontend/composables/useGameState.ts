...
export function useGameState() {
  ...
  function openMenu(m: any) { activeMenu.value = m; activeSub.value = m.children?.[0]?.key || m.key; modalDesc.value = descs[activeSub.value] || ''; if (!['world-map', 'backpack'].includes(m.key)) modalVisible.value = true }
  ...
  async function loadPlayer() { const pid = getPID(); if (!pid) return; try { const t = getToken(); const r = await fetch('/api/v1/player/' + pid, { headers: { Authorization: 'Bearer ' + t } }); const d = await r.json(); if (d.data) { const p = d.data.player || d.data; Object.assign(player, p); if (d.data.spirit_name||d.data.spiritName) player.spiritName = d.data.spirit_name||d.data.spiritName; if (d.data.realm_name) player.realmName = d.data.realm_name; if (d.data.realm_stage) player.realmStage = d.data.realm_stage; if (d.data.quality_name) player.qualityName = d.data.quality_name; if (typeof player.critRate === 'number' && player.critRate > 100) player.critRate = Math.round(player.critRate/100); if (typeof player.critDmg === 'number' && player.critDmg > 1000) player.critDmg = Math.round(player.critDmg/100); if (typeof player.dodge === 'number' && player.dodge > 100) player.dodge = Math.round(player.dodge/100); if (typeof player.hit === 'number' && player.hit > 100) player.hit = Math.round(player.hit/100); if (typeof player.mpRegen === 'number' && player.mpRegen > 100) player.mpRegen = Math.round(player.mpRegen/100) } } catch {} }
