<template>
  ...
  <div class="gold-divider"/><header class="top-bar" style="border-radius:8px 8px 0 0"><div class="top-bar-inner"><div class="top-bar-spacer"/><span class="brand-name">🕊️ 飞鸽传书</span><div class="top-bar-spacer"/><button class="modal-close" @click="$emit('close')">✕</button></div></header><div class="gold-divider"/>
  ...
  <div class="pig-left">
    ...
    <div v-if="searchResults.length"><div v-for="u in searchResults" class="pig-sr-row" @click="addFriend(u.id)"><span class="pig-sr-name">{{ u.nickname }}</span><van-button size="mini" type="primary">加好友</van-button></div></div>

    <div class="pig-group"><div class="pig-group-head" @click="showSect=!showSect">🏛️ 宗门 ▾</div>
    <div v-if="showSect"><div v-if="sectList.length"><div v-for="s in sectList" class="pig-contact" :class="{active:activeChat===s.id}" @click="openSectChat(s)"><span class="pig-contact-avatar">{{ s.name[0] }}</span><span class="pig-contact-name">{{ s.name }}</span></div></div><div v-else class="pig-empty-tip">未加入宗门</div></div>
    </div>

    <div class="pig-group"><div class="pig-group-head" @click="showFriends=!showFriends">👫 好友 {{ friends.length }} ▾</div>
    <div v-if="showFriends"><div v-if="friends.length"><div v-for="f in friends" class="pig-contact" :class="{active:activeChat==='f_'+f.id}" @click="openFriendChat(f)"><span class="pig-contact-avatar">{{ f.nickname[0] }}</span><span class="pig-contact-name">{{ f.nickname }}</span></div></div><div v-else class="pig-empty-tip">暂无好友</div></div>
    </div>

    <div class="pig-group"><div class="pig-group-head" @click="showDaolv=!showDaolv">💑 道侣 ▾</div>
    <div v-if="showDaolv"><div v-if="daolvPartner" class="pig-contact" :class="{active:activeChat==='daolv'}" @click="openDaolvChat"><span class="pig-contact-avatar">💑</span><span class="pig-contact-name">道侣</span></div><div v-else class="pig-empty-tip">修仙路漫漫</div></div>
    </div>

    <div class="pig-group"><div class="pig-group-head" @click="showMaster=!showMaster">👴 师父 ▾</div>
    <div v-if="showMaster"><div v-if="masterName" class="pig-contact" :class="{active:activeChat==='master'}" @click="openMasterChat"><span class="pig-contact-avatar">👴</span><span class="pig-contact-name">{{ masterName }}</span></div><div v-else class="pig-empty-tip">尚未拜师</div></div>
    </div>

    <div class="pig-group"><div class="pig-group-head" @click="showDisciple=!showDisciple">🧒 徒儿 ▾</div>
    <div v-if="showDisciple"><div v-if="discipleList.length"><div v-for="d in discipleList" class="pig-contact" :class="{active:activeChat==='d_'+d.id}" @click="openDiscipleChat(d)"><span class="pig-contact-avatar">{{ d.nickname[0] }}</span><span class="pig-contact-name">{{ d.nickname }}</span></div></div><div v-else class="pig-empty-tip">尚未收徒</div></div>
    </div>
  ...
  <div class="pig-right">
    <div v-if="activeChat" class="pig-chat">
      ...
      <div class="pig-chat-msgs" ref="chatMsgs"><div v-for="m in currentMessages" class="pig-msg-bubble" :class="{self:m.sender_id===myId||m.name===myName}"><div class="pig-msg-avatar">{{ (m.sender_name||m.name||'?')[0] }}</div><div class="pig-msg-bub-text">{{ m.content||m.text }}</div></div></div>
      <div class="pig-chat-send"><input v-model="chatInput" @keyup.enter="sendMsg" placeholder="输入..." class="pig-send-inp"/><button @click="sendMsg" class="pig-send-btn">发送</button></div>
    </div>
    <div v-else class="pig-empty-tip" style="flex:1;display:flex;align-items:center;justify-content:center">选择一个联系人或频道</div>
  </div>
...
const chatTitle=computed(()=>{if(activeChat.value==='world')return'🌍 世界频道';if(activeChat.value==='daolv')return'💑 道侣';if(activeChat.value==='master')return'👴 师父';if(activeChat.value?.startsWith('d_')){const d=discipleList.value.find((x:any)=>'d_'+x.id===activeChat.value);return'🧒 '+d?.nickname}const f=friends.value.find((x:any)=>'f_'+x.id===activeChat.value);if(f)return f.nickname;const s=sectList.value.find((x:any)=>x.id===activeChat.value);if(s)return s.name;return'聊天'})
...
async function addFriend(fid:number){await api('/api/v1/player/'+playerId.value+'/friends/add',{method:'POST',body:JSON.stringify({friend_id:fid})});searchResults.value=[];loadFriends()}
...
function openFriendChat(f:any){activeChat.value='f_'+f.id;currentMessages.value=[]}
function openSectChat(s:any){activeChat.value=s.id;currentMessages.value=[]}
function openDaolvChat(){activeChat.value='daolv';currentMessages.value=[]}
function openMasterChat(){activeChat.value='master';currentMessages.value=[]}
function openDiscipleChat(d:any){activeChat.value='d_'+d.id;currentMessages.value=[]}
...
.pig-left{width:240px;min-width:180px;overflow-y:auto;border-right:1px solid rgba(212,168,67,.1);background:rgba(0,0,0,.2)}
...
.pig-group-head{padding:9px 12px;font-size:12px;font-weight:600;color:rgba(255,255,255,.55);cursor:pointer;user-select:none}
.pig-group-head:hover{background:rgba(255,255,255,.03);color:#d4a843}
.pig-contact{display:flex;align-items:center;gap:8px;padding:6px 12px;cursor:pointer}.pig-contact:hover{background:rgba(212,168,67,.05)}.pig-contact.active{background:linear-gradient(90deg,rgba(212,168,67,.12),rgba(212,168,67,.04));border-right:2px solid #d4a843}
.pig-contact-avatar{width:28px;height:28px;border-radius:50%;background:linear-gradient(135deg,#d4a843,#b8860b);display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:700;color:#fff;flex-shrink:0}
...
.pig-msg-avatar{width:26px;height:26px;border-radius:50%;background:rgba(255,255,255,.1);display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700;color:rgba(255,255,255,.6);flex-shrink:0}
