  ...
  <Teleport to="body">
    <div v-if="visible" class="modal-overlay" @click.self="visible=false">
      <div class="wiki-modal" style="width:700px;max-width:95vw">
        <div class="gold-divider"/><header class="top-bar" style="border-radius:8px 8px 0 0"><div class="top-bar-inner"><div class="top-bar-spacer"/><span class="brand-name" style="font-size:16px">🎒 背包</span><div class="top-bar-spacer"/><button class="modal-close" @click="visible=false">✕</button></div></header><div class="gold-divider"/>
        <div class="wiki-body">
          ...
          <div v-else style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:8px">
            <div v-for="item in items" :key="item.id" class="pill-card" style="cursor:pointer" @click="selected=item">
...
async function load() {
  try {
    const r = await fetch('/api/v1/player/' + playerId.value + '/inventory', { headers: { Authorization: 'Bearer ' + token.value } })
    const d = await r.json(); items.value = d.data || []
  } catch {}
}
async function useItem(item: any) {
  await fetch('/api/v1/player/' + playerId.value + '/items/use', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: 'Bearer ' + token.value }, body: JSON.stringify({ item_id: item.id }) })
  load()
}
