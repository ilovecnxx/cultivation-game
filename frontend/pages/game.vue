<template>
  <div class="gh-root" :class="{ 'light-mode': !isDark }">
    ...
    <Teleport to="body">
      <van-popup v-model:show="modalVisible" position="center" :style="{ width:'420px; max-width:90vw; border-radius:8px; background:#1a1a2e' }" v-if="activeMenu?.key!=='map'&&activeMenu?.key!=='profession'&&activeMenu?.key!=='pigeon'&&activeMenu?.key!=='backpack'">
        <div class="modal-card">
          <div class="modal-header"><h2>{{ activeMenu?.label }}</h2>✕</div>
    ...
    <Teleport to="body">
      <div v-if="showWiki" class="modal-overlay" @click.self="showWiki=false">
        <div class="wiki-modal">
          <div class="wiki-header"><h2>百科</h2><button class="modal-close" @click="showWiki=false">✕</button></div>
          <div class="wiki-tabs">
